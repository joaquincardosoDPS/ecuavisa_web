...
function ProfileDropdown() {
  ...
  return (
    <div className="relative pr-10 xl:pr-25 pt-3.5" ref={dropdownRef}>
      ...
      >
        <span className="z-30 text-(--clr-primary-title) font-medium group-hover:text-(--foc-primary) transition-colors duration-200">
          Mi perfil
        </span>
        <ProfileAvatar
          ...
          <div className="flex flex-col mb-5">
            {profiles?.map((profile) => (
              <button
                ...
                className={twMerge(
                  "flex items-center gap-3 px-5 py-2.5 cursor-pointer hover:bg-(--clr-primary-title)/10 transition-all duration-150",
                )}
              >
                <ProfileAvatar profile={profile} size="sm" />
                <span className="text-(--clr-primary-title) text-sm font-medium">
                  {profile.name_perfil}
                </span>
              </button>
            ))}
            ...
            {(profiles?.length ?? 0) < 4 && (
              <button
                ...
                className="flex items-center gap-3 px-5 py-2.5 cursor-pointer hover:bg-(--clr-primary-title)/10 transition-all duration-150"
              >
                <div className="w-10 h-10 rounded-full bg-(--clr-primary-title)/10 flex items-center justify-center shrink-0">
                  <span className="text-xl text-(--clr-primary-title)/50">+</span>
                </div>
...
function DropdownLink({
  ...
  return (
    <button
      onClick={onClick}
      className="px-5 py-2 text-left text-sm text-(--clr-primary-title)/70 hover:text-(--clr-primary-title) hover:bg-(--clr-primary-title)/10 transition-all duration-150 cursor-pointer"
    >
      {children}
    </button>
  );
